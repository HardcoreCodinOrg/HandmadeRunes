...
resize :: proc(new_width, new_height: i32) {
	resizeFrameBuffer(&frame_buffer, new_width, new_height);
	update();	
}
...
update :: proc() {
	...
	if len(stored_bounds) > 0 do
		for current_stored_bounds in &stored_bounds do
			drawBounds(&frame_buffer.bitmap, &current_stored_bounds, &RED);

	if buttons.left.is_pressed {
		drawBitmap(&brush, &canvas.bitmap, {
			mouse.pos.x - brush.width/2,
			mouse.pos.y - brush.height/2}
		);
		...
		// drawBounds(&frame_buffer.bitmap, &bounds, &WHITE);
	}
	...
	if buttons.right.is_pressed {
		...
		drawBounds(&frame_buffer.bitmap, &bounds, &RED);
...
render :: proc() {
	...
	before = getTicks();
	drawWeirdColors();
	drawBitmap(&canvas.bitmap, &frame_buffer.bitmap, {0,0});

	drawBitmap(&brush, &frame_buffer.bitmap, {
		mouse.pos.x - brush.width/2,
		mouse.pos.y - brush.height/2});
