...
WndProc:: proc "std" (hWnd: HWND, message: UINT, wParam: WPARAM, lParam: LPARAM) -> LRESULT {
	...
	switch message {
		...
		// case WM_MOUSEMOVE: mouseMoved(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		...
		case WM_MBUTTONUP:   
			setMouseButtonState(&mouse.buttons.middle, false);
			ReleaseCapture();
			ShowCursor(true);

		case WM_MBUTTONDOWN: 
			setMouseButtonState(&mouse.buttons.middle, true);
			SetCapture(window);
			ShowCursor(false);

		case WM_LBUTTONDOWN: 
			setMouseButtonState(&mouse.buttons.left, true);

		case WM_LBUTTONUP:   
			setMouseButtonState(&mouse.buttons.left, false);

		case WM_RBUTTONDOWN: 
			setMouseButtonState(&mouse.buttons.right, true);

		case WM_RBUTTONUP:   
			setMouseButtonState(&mouse.buttons.right, false);
